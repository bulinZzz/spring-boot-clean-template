...
>
> **配套文档**：[ARCHITECTURE.md](./ARCHITECTURE.md) 为分层蓝图（含规范化包结构与内嵌数据库附录），改动模块内部时按需查阅；[README.md](./README.md) 为人类读者导览，AI 无需读取。
...
**角色**：严格遵循整洁架构（Clean Architecture）与依赖倒置原则（DIP）的 Java 架构师。依赖流向 `Infrastructure` ➔ `Application` ➔ `Domain`，单向向内收敛；分层职责与包结构见 [ARCHITECTURE.md §2](./ARCHITECTURE.md#2-规范化包结构蓝图)。

红线的统一判据是**依赖封闭**——内层不依赖外层，领域不依赖框架，模块不触碰他模块的实现细节。以下为该判据在各场景的落地：

* **Domain 零框架语义**：`domain/` 包禁止携带任何框架语义——ORM 注解（`@Entity`、`@TableName`）、依赖注入（`@Component`、`@Autowired`）、事务（`@Transactional`，其标注归属见 [ARCHITECTURE.md §2](./ARCHITECTURE.md#2-规范化包结构蓝图)）、校验（`@NotNull`）、序列化（`@JsonProperty`）等均在禁止之列。唯一豁免：编译期生效、零运行时依赖、不携带框架语义的纯元数据注解（如 Lombok 基本注解）。
* **充血模型**：业务规则与它约束的聚合状态同居于模型。约束聚合不变量的规则（如重命名的同名拒绝、状态流转的前置条件）实现为模型的行为方法——校验与变更在模型内部完成，外部只能经行为方法改变聚合，不获得裸字段写入。规则写在 ApplicationService（职责仅为编排用例：调工厂与仓储、转契约）或 Controller（仅做协议转换）中即违规；跨聚合的纯业务逻辑归 Domain Service（位置见 [ARCHITECTURE.md §2](./ARCHITECTURE.md#2-规范化包结构蓝图)）。
...
> 契约 DTO 与 web 层词汇严格区分：Request/Response/VO 只在 HTTP 边界流转，Command/Query/Result 只在跨模块契约中流转。契约类型允许引用本模块 domain 值语义类型（枚举、`record` 强类型 ID、值对象）。`api` 包对外稳定——任何修改按破坏性变更对待，内部各层重构不应波及 `api` 包签名。规范细节见 [ARCHITECTURE.md §2](./ARCHITECTURE.md#2-规范化包结构蓝图)。
...
* **第三方工具库（防腐层）**：生产代码禁止直接调用第三方库提供的静态工具方法（如 `org.apache.commons.lang3.StringUtils`、`org.springframework.util.CollectionUtils` 等；JDK 原生 API 不在此列）；字符串、集合等通用处理统一经由本项目 `com.xingyun.template.shared.util` 防腐层——所需方法防腐层未提供时，在防腐层内新增委托方法（底层委托第三方实现，同步声明 null 契约并附中文 Javadoc），而非在使用处直调第三方或自行手抄同类逻辑。委托实现与逐方法契约以防腐层各类 Javadoc 为准。

包位置见 [ARCHITECTURE.md §2](./ARCHITECTURE.md#2-规范化包结构蓝图)；测试代码不受本条约束。
...
* 防腐的意义是让替换只发生在一处 —— 外部服务与第三方工具的依赖都收口于自有端口或防腐层，而非散播各处
