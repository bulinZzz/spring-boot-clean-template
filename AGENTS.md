...
注释只表达代码本身无法充分体现、且对理解当前代码具有稳定价值的信息。
...
能够从命名、类型、结构或控制流直接看懂的信息，不写注释。

不要为了“完整”给每个类、方法或字段机械添加 Javadoc。
...
注释应描述当前仍然有效的稳定事实或设计意图，而不是作者的思考过程。

#### 注释位置

* 类级 Javadoc：说明类职责、边界或稳定设计意图。
* 方法 Javadoc：说明签名无法表达、且调用者确实需要知道的契约、前置条件、业务规则或副作用。
* 行注释：只解释紧邻代码的非显然原因，不重复代码行为。
