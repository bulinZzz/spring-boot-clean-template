...
> 契约 DTO 与 web 层词汇严格区分：Request/Response/VO 只在 HTTP 边界流转，Command/Query/Result 只在跨模块契约中流转。契约类型允许引用本模块 domain 值语义类型（枚举、`record` 强类型 ID、值对象）。`api` 包是模块的公开目录（"读 api 包 = 理解模块"），对外稳定——任何修改按破坏性变更对待，内部各层重构不应波及 `api` 包签名。规范细节见 [ARCHITECTURE.md §2](./ARCHITECTURE.md#2-规范化包结构蓝图)。
...
仓储与出站端口是依赖倒置的对称双端：**接口定义在内层**——仓储属 `domain`、出站端口属 `application`，均为纯 Java Interface；**技术实现在外层** `infrastructure`——持久化实现经 `Converter` 完成 PO 与领域模型互转，集成实现封装外部服务调用。内层只依赖自己声明的抽象，不感知技术细节。

包位置见 [ARCHITECTURE.md §2](./ARCHITECTURE.md#2-规范化包结构蓝图)，标准代码见 [ARCHITECTURE.md §3](./ARCHITECTURE.md#3-标准代码规范)。
...
全项目禁止在业务代码中直接使用第三方工具类（如 `StringUtils`、`CollectionUtils`、`DateUtils` 等）；统一经由本项目 `com.xingyun.template.shared.util` 防腐层，其委托实现、方法契约与分层使用细则以各类 Javadoc 为准。核心思想：
...
* 防腐层的意义是让替换只发生在一处 —— 依赖集中收口，而非散播各处
