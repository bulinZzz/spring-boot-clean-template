...
注释与文档修改后不留过程痕迹——删改与方案变更一律按最终意图重新表达，成品读来应如初次写下；历史追溯交给 Git。

### 任务收尾审计

编码完成不是任务完成的标志。每个任务收尾时，改动先逐条对照 §2 红线与 §6 约定过审，修复后自测，自测通过才算收尾。
...
* 可观察信号胜过身份猜测 —— 行为由输入与状态决定，不由对参与者的预设决定
