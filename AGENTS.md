...
* 没有差异化处理需求时，优先使用标准 JDK / Spring 异常。
* 需要表达稳定业务语义时，使用 `BusinessException`。
* `BusinessException` 是默认的业务异常类型，不应因为业务场景名称不同而机械新增异常类型。
* `BusinessException` 只表达业务语义，不携带 HTTP 状态码、HTTP 响应类型或其他外部协议语义。
* 只有当现有异常类型无法满足明确的差异化处理需求时，才新增异常类型。差异化处理包括不同的捕获、日志、恢复、重试、补偿或外部协议映射行为。
* 业务代码通常不应捕获异常后仅为了重新包装或重新映射而再次抛出；异常应尽可能自然传播到能够正确处理它的边界。
* Web 层通过 `GlobalExceptionHandler` 将业务异常统一翻译为 HTTP `ProblemDetail`。
* 未预期异常由全局异常处理器统一兜底；完整异常信息应进入日志，但不得将内部实现细节直接暴露给调用方。
...
根据变更范围选择验证方式，并优先扩展现有、职责匹配的测试类；只有现有测试没有合适归属，或新增验证形成独立测试责任时，才创建新的测试类。生产类与测试类不要求一一对应。
