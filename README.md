...
3. **启动验证**：执行 `mvn spring-boot:run`，服务基于 H2 内存库启动，无需准备外部数据库。
4. **开发前准备**：建议先阅读 ARCHITECTURE.md 与 AGENTS.md，对架构边界建立整体认识。
5. **搭建模块**：可参照 `module/example/` 搭建业务模块；该示例模块不含业务逻辑，如不需要，可整体移除。

## 文档导航

| 文档 | 内容 | 读者 |
| :--- | :--- | :--- |
| [ARCHITECTURE.md](./ARCHITECTURE.md) | 分层架构哲学、包结构蓝图、标准代码规范 | 所有开发者 |
