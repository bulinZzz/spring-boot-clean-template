...
模板内置两个可整体移除的教学模块（`example` 与消费其契约的 `comment`），以及支撑它们开箱即跑的内嵌 H2 数据库。范例用于演示架构边界和典型写法，不承载模板使用者的实际业务，不需要时按本文「移除范例」与「替换或移除 H2」章节清理。

> 架构蓝图见 [ARCHITECTURE.md](../ARCHITECTURE.md)；AI 工作规则见 [AGENTS.md](../AGENTS.md)。
...
`module/comment/` 演示模块封装边界——仅依赖 `example` 的 `api` 包契约及其签名所需的稳定值语义类型（`ExampleId`），不触碰 `example` 的内部实现；`example_id` 仅作为逻辑关联，不建立物理外键。

## H2：范例的运行时存储

范例的仓储链路需要真实数据库才能跑通，但模板不预设使用者的数据库环境。模板内置 **H2**——可随应用进程内嵌启动的轻量级数据库，以**内存模式**运行：库表与数据只存活于应用进程内，启动时重建、停止时消失，零安装、零外部配置。
...
建表脚本归属范例，借 H2 的内嵌数据库初始化机制落地；移除范例时一并删除。接入真实数据库后，表结构初始化应交由迁移工具（如 Flyway、Liquibase）或 DBA 流程管理。

### 使用：查看内存库数据

1. **启动应用**：启动项目后，浏览器访问 `http://localhost:8080/h2-console`。
2. **填写连接信息**：登录页连接信息与 `application.yml` 的 `spring.datasource` 配置一致。JDBC URL 填内存库地址 `jdbc:h2:mem:example`（控制台连接不带 `DB_CLOSE_DELAY` 参数），用户名与密码照配置填写。
3. **查看数据**：连接后即可查看示例与评论表（脚本中写作 `t_example`、`t_comment`；H2 对未加引号的标识符按大写存储，控制台中显示为 `T_EXAMPLE`、`T_COMMENT`）。调用 `POST /examples`、`POST /comments` 写入的数据即时可见。

内存库只存活于应用进程内：IDEA 数据库工具、DBeaver 等外部客户端无法连接——在另一个 JVM 中使用同一 URL 只会新建一个同名的空库；应用停止，数据即失。

## 移除范例

范例仅用于教学和架构演示，不属于模板使用者的实际业务代码；不需要时按下列步骤删除：
...
3. **脚本**：接入真实数据库后，不再依赖 `schema.sql` 作为生产建表机制；真实库建表交由迁移工具（如 Flyway、Liquibase）或 DBA 流程管理。当前范例 PO 使用 `IdType.AUTO`；若沿用该映射，数据库主键应采用相应的自增策略。
